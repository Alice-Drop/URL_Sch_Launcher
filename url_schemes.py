...
for item in data_list:
    if item:
        if not "://" in item:
            # 说明是标题。
            current_title = item.strip()
        else:
            splited = item.split(" ")
            if len(splited) >1:
                current_part[splited[0]] = splited[1]
                # output_data.append({splited[0]: splited[1]})
    else:
        # 是空行，表示一个类（即一个App）结束了
        if current_title:
            output_data[current_title] = current_part
        else:
            for key,value in current_part.items():
                output_data[key] = {key:value}

        # 存储完成，开始抹除，开启下一个App
        current_part = {}
        current_title = ""

with open("./output.json", "w", encoding="utf-8") as my_file:
    json.dump(output_data, my_file, indent=4, ensure_ascii=False)  	    		
